...
namespace Vasters.ForzaBridge.ProducerData.ForzaMotorsport.Telemetry
{
    ...
    public partial class BatchTimespan : global::Avro.Specific.ISpecificRecord
    {
        ...
        ///</summary>
        public BatchTimespan(global::Avro.Generic.GenericRecord obj)
        {
        ...
        public static BatchTimespan? FromData(object? data, string? contentTypeString )
        {
            ...
            if ( contentType.MediaType.StartsWith("avro/") || contentType.MediaType.StartsWith("application/vnd.apache.avro") )
            {
                ...
                {
                    var reader = new global::Avro.Generic.GenericDatumReader<global::Avro.Generic.GenericRecord>(BatchTimespan.AvroSchema, BatchTimespan.AvroSchema);
                    return new BatchTimespan(reader.Read(null, new global::Avro.IO.BinaryDecoder(stream)));
                }
                if ( contentType.MediaType.StartsWith("avro/json") || contentType.MediaType.StartsWith("application/vnd.apache.avro+json"))
                {
                    var reader = new global::Avro.Generic.GenericDatumReader<global::Avro.Generic.GenericRecord>(BatchTimespan.AvroSchema, BatchTimespan.AvroSchema);
                    return new BatchTimespan(reader.Read(null, new global::Avro.IO.JsonDecoder(BatchTimespan.AvroSchema, stream)));
                }
            ...
            if ( contentType.MediaType.StartsWith(System.Net.Mime.MediaTypeNames.Application.Json))
            {
                ...
                {
                    return System.Text.Json.JsonSerializer.Deserialize<BatchTimespan>(new ReadOnlySpan<byte>((byte[])data));
                }
                else if (data is System.IO.Stream)
                {
                    return System.Text.Json.JsonSerializer.DeserializeAsync<BatchTimespan>((System.IO.Stream)data).Result;
                }
            }
            throw new System.NotSupportedException($"Unsupported media type {contentType.MediaType}");
        }
        private class SpecificDatumWriter : global::Avro.Specific.SpecificDatumWriter<BatchTimespan>
        {
            public SpecificDatumWriter() : base(BatchTimespan.AvroSchema)
            {
            }
        ...
        public byte[] ToByteArray(string contentTypeString)
        {
            ...
            {
                var stream = new System.IO.MemoryStream();
                var writer = new global::Avro.Specific.SpecificDatumWriter<BatchTimespan>(BatchTimespan.AvroSchema);
                var encoder = new global::Avro.IO.JsonEncoder(BatchTimespan.AvroSchema, stream);
                writer.Write(this, encoder);
                encoder.Flush();
                result = stream.ToArray();
            }
